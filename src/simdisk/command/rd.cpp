...
int DiskManager::rd(int argc, char *argv[], int inode_id, int block_id) {
	...
	if (argc != 0) {
		...
		if (optind == argc) {
			std::cerr << "rd: missing operand" << std::endl;
			return -1;
		}
		path = argv[optind];
		...
		if (inode->m_type != FileType::DIRECTORY) {
			std::cerr << "rd: cannot delete '" << filename << "': Not a directory" << std::endl;
			return -1;
		}
	...
	for (auto &[name, d] : file->m_dirs) {
		...
		if (sub_inode->m_type == FileType::DIRECTORY)
			rd(0, nullptr, d.m_inode, sub_inode->m_location);
		else
			del(0, nullptr, d.m_inode, sub_inode->m_location);
	}
