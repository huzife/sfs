...
void Initializor::setDiskManager(std::shared_ptr<DiskManager> mgr) {
    disk_manager = mgr;
}
...
void Initializor::initSuperBlock() {
    ...
    s.m_free_inode = DiskManager::file_block_count - 1;
...
void Initializor::initFAT() {
    ...
    for (int i = 0; i < DiskManager::fat_block_count; i++) {
        ...
        disk_manager->writeBlock(i + 1, block);
    }
}
...
void Initializor::initAllocMap() {
    ...
    for (int i = 0; i < 12; i++) {
        ...
        disk_manager->writeBlock(i + 401, block1);
        disk_manager->writeBlock(i + 413, block2);
        if (i == 0) {
            buffer[0] = 0;
        }
    }
...
void Initializor::initRoot() {
    ...
    disk_manager->writeBlock(DiskManager::inode_block_offset, inode_block);
    ...
    disk_manager->writeBlock(DiskManager::file_block_offset, file_block);
}